...
class ReplayBuffer:
    ...
    def __init__(self, obs_dim, act_dim, size):
        self.obs_buf = np.zeros(sac_core.combined_shape(size, obs_dim), dtype=np.float32)
        self.obs2_buf = np.zeros(sac_core.combined_shape(size, obs_dim), dtype=np.float32)
        self.act_buf = np.zeros(sac_core.combined_shape(size, act_dim), dtype=np.float32)
...
def sac(num_envs, env_fn, actor_critic=sac_core.MLPActorCritic, ac_kwargs=dict(), seed=0, 
    ...
    # var_counts = tuple(sac_core.count_vars(module) for module in [ac.pi, ac.q1, ac.q2])
...
#     sac(lambda : gym.make(args.env), actor_critic=sac_core.MLPActorCritic,
